...
const TiptapEditor = forwardRef<TiptapEditorRef, TiptapEditorProps>(
  (props, ref) => {
    ...
    // Sync external content prop into the editor when it changes (e.g., on edit)
    useEffect(() => {
      if (!editor) return;
      if (typeof content === 'undefined' || content === null) return;
      // Use emitUpdate: false to avoid triggering onChange loop
      try {
        editor.commands.setContent(content as Content, { emitUpdate: false });
      } catch (e) {
        // noop: if setContent fails due to malformed input, do not crash
      }
